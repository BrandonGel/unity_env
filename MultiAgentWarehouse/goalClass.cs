...
namespace multiagent.goal
{
    public class goalClass
    {
        ...
        {
            UnityEngine.Assertions.Assert.IsTrue(robotID >= 0);
            anyAssigned[robotID] = false;
            bool allArrayElementsFalse = anyAssigned.All(b => b == false);
            if (allArrayElementsFalse)
            {
                Util.enableRenderer(this.goalObj.GetComponent<Renderer>(), false);    
            }
        }

        public void reset(int numAgents = 1)
        {
            anyAssigned = new bool[numAgents];
            Array.Fill(anyAssigned, false);
            Util.enableRenderer(this.goalObj.GetComponent<Renderer>(), false);
        }
